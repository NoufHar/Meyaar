    ...
    <div className="min-h-screen bg-[#f4f7fb] text-slate-950">
      ...
      <div className="lg:pl-56 rtl:lg:pl-0 rtl:lg:pr-56">
        ...
        <main className="mx-auto max-w-[1500px] p-4 lg:p-5">
          ...
          {result && view === "analysis" && showBatchOverview && batchUploads.length > 1 && <BatchOverview items={batchUploads} onOpen={(item) => { setResult(item.result); setSelectedErrorId(null); setSearch(""); setSeverity("all"); setErrorType("all"); setImageUrl((current) => { if (current) URL.revokeObjectURL(current); return item.mode === "image" ? URL.createObjectURL(item.file) : null; }); setShowBatchOverview(false); }} />}
          {result && view === "analysis" && !showBatchOverview && <div className="space-y-6">{batchUploads.length > 1 && <button type="button" onClick={() => setShowBatchOverview(true)} className="rounded-xl border border-blue-200 bg-white px-4 py-2 text-sm font-bold text-blue-700">← Back to uploaded files</button>}{filters}{vectorResult ? <MapPanel result={displayedResult ?? vectorResult} selectedErrorId={selectedErrorId} /> : <VisionPreview result={result as VisionAnalysisResponse} imageUrl={imageUrl} />}<ErrorTable result={displayedResult ?? result} selectedErrorId={selectedErrorId} onSelectError={vectorResult ? selectError : undefined} /></div>}
